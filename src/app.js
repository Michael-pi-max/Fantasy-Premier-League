...
const mosttransferredinplayer = document.querySelector(
...
let view = async function () {
  ...
  gameweekTime.textContent = new Date(
    events[dataController.gameweekpage].deadline_time
  ).toDateString();
  averageScore.textContent =
    events[dataController.gameweekpage].average_entry_score;
  highestScore.textContent = events[dataController.gameweekpage].highest_score;

  //TODO
  mosttransferredin.textContent =
    elements[events[dataController.gameweekpage].most_transferred_in]
      .first_name +
    " " +
    elements[events[dataController.gameweekpage].top_element].second_name;
  mosttransferredinplayer.innerHTML = `<img src=${
    teams[
      elements[events[dataController.gameweekpage].most_transferred_in].team
    ].jersey
  }>`;
  mostcaptainedin.textContent =
    elements[events[dataController.gameweekpage].most_captained].first_name +
    " " +
    elements[events[dataController.gameweekpage].most_captained].second_name;
  mostcaptainedinplayer.innerHTML = `<img src=${
    teams[elements[events[dataController.gameweekpage].most_captained].team]
      .jersey
  }>`;

  // Starplayer
  starplayertitle.textContent = `GW ${events[5].id} star player`;
  starplayerjersey.innerHTML = `<img src=${
    teams[elements[events[dataController.gameweekpage].top_element].team].jersey
  }>`;
  starplayerfirstname.textContent =
    elements[events[dataController.gameweekpage].top_element].first_name;
  starplayerlastname.textContent =
    elements[events[dataController.gameweekpage].top_element].second_name;
  starplayerteam.textContent =
    "⚪ " +
    teams[elements[events[dataController.gameweekpage].top_element].team - 1]
      .name;
  starplayerpoints.textContent =
    events[dataController.gameweekpage].top_element_info.points +
    " " +
    "points";
            ...
            <div class="col-sm-12 fixturebox py-3">
                <div class="row mt-2">
                    ...
                    <div class="col-sm-2 versus text-center py-2">
                        ${
                          fixtures[i].team_a_score !== null &&
                          fixtures[i].team_h_score !== null
                            ? fixtures[i].team_h_score +
                              "-" +
                              fixtures[i].team_a_score
                            : "V"
                        }
                    </div>
                    ...
                    <div class="col-sm-5 fixtureplacetime">
                        <div class="row">
                            <div class="col-sm-12 fixturetime">${
                              new Date(fixtures[i].kickoff_time).getHours() +
                              ":" +
                              new Date(fixtures[i].kickoff_time).getMinutes()
                            }</div>
                            <div class="col-sm-12 fixtureplace text-muted">${
                              teams[fixtures[i].team_h - 1].arena
                            }</div>
                        </div>
                    </div>
                </div>
